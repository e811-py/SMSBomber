...
#</###########UpToLicense#########################################\>
upToLicense_GetID, upToLicense_GetLong, upToLicense_OK = range(3)
...
def UpToLicense_GetID(update, context):
    try:
        ...
        update.message.reply_text('برای چه مدت کاربر ویژه شود؟', reply_markup=ReplyKeyboardMarkup([['هفتگی', 'ماهانه'], ['همیشگی']]))
        # update.message.reply_text("آیا مطمعن هستید؟", reply_markup=ok_markup)

        return upToLicense_GetLong
    except:
        update.message.reply_text('نامعتبر است.')

def UpToLicense_GetLong(update, context):
    global upToVipMode, mode
    if update.message.text == 'هفتگی':
        upToVipMode = 604800
        mode = 'week'
        update.message.reply_text('آیا مطمعن هستید؟', reply_markup=ok_markup)
        return upToLicense_OK

    elif update.message.text == 'ماهانه':
        upToVipMode = 2592000
        mode = 'mounth'
        update.message.reply_text('آیا مطمعن هستید؟', reply_markup=ok_markup)
        return upToLicense_OK

    elif update.message.text == 'همیشگی':
        upToVipMode = 'Never'
        mode = 'ever'
        update.message.reply_text('آیا مطمعن هستید؟', reply_markup=ok_markup)
        return upToLicense_OK
...
def UpToLicense_OK(update, context):
    if update.message.text == 'تایید':
        for i in Users.find({'_id':UpToLicenseID}): bm=i['VipMode']['pay']
        if not bm:
            if mode == 'week':
                wd = int(str(date.today()).split('-')[2]) + 7
                Users.find_one_and_update({'_id': UpToLicenseID}, {"$set": {"VipMode": {'pay': True, 'End': time()+upToVipMode, 'date': f'{str(date.today()).split("-")[0]}-{str(date.today()).split("-")[1]}-{wd}'}}})
            elif mode == 'mounth':
                wd = int(str(date.today()).split('-')[1]) + 1
                Users.find_one_and_update({'_id': UpToLicenseID}, {"$set": {"VipMode": {'pay': True, 'End': time()+upToVipMode, 'date': f'{str(date.today()).split("-")[0]}-{wd}-{str(date.today()).split("0")[2]}'}}})
            elif mode == 'ever':
                Users.find_one_and_update({'_id': UpToLicenseID}, {"$set": {"VipMode": {'pay': True, 'End': 'Never', 'date': ''}}})

            # try: twrite = time()+upToVipMode
            # except: twrite = 'Never'
            # Users.find_one_and_update({'_id':UpToLicenseID}, {"$set":{"VipMode":{'pay':True, 'End':twrite, 'data':f'{str(date.today()).split("-")[0]}-{str(date.today()).split("-")[1]}-{wd}'}}})
...
UpToLicense_conv = ConversationHandler(
    ...
    states={
        ...
        upToLicense_GetLong : [MessageHandler(Filters.text & ~Filters.regex('^لغو$'), UpToLicense_GetLong)],
...
def DownToLicense_OK(update, context):
    if update.message.text == 'تایید':
        Users.find_one_and_update({'_id':DownToLicenseID}, {'$set': {'VipMode':{'pay':False, 'End':-9999}}})
