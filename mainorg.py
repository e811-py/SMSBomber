...
# logChannel_id = -1001358273419 ##test
...
# token = '1168215468:AAHTHcQVwL6jMN2VdUnpEZf79jS84hiyu-8'
# token = '1379182862:AAEvDxfnciWxVSy49f02aZGLKVviq3BzpR0'
...
def start(update, context):
    ...
    try:
        ...
        Users.insert_one({"_id":int(update.message.chat_id), "Username":update.message.from_user.username, "Requests":[], "Subset":0, "VipMode":{'pay':False, 'End':'Never', 'date':''}, "TestVip":False, "Ban":False, "lastF":{"time":timee, "date":f"{date.today()}"}, "lastLoc":"main_menu"})
        bmm = False
        req = False
    except:
        # global bmm, req
    ...
    if not bmm:
        if context.args != []:
            ...
            if not req:
                ...
                if sub >= 50:
                    Users.find_one_and_update({"_id": subadd}, {'$set': {'VipMode': {'pay':True, 'End':'Never', 'date':''}}})
...
def VipMood_Start(update, context):
    ...
    if not bmm:
        if context.bot.get_chat_member(-1001144465509, update.message.chat_id).status == 'left':
            ...
            for i in Users.find({'_id':int(update.message.chat_id)}): li=i['VipMode']['pay']
            if not li:
...
def info(update, context):
    ...
    if context.bot.get_chat_member(-1001144465509, update.message.chat_id).status == 'left':
        ...
        for i in Users.find({'_id':int(update.message.chat_id)}):
            ...
            if not i['VipMode']['pay']: sh_msg += "🔰بخش ویژه: غیر فعال\n"
            elif i['VipMode']['pay']:
                if i['VipMode']['End'] == 'Never': sh_msg += f"🔰بخش ویژه: فعال (تا تاریخ: همیشه)\n"
                else:
                    sh_msg += f"🔰بخش ویژه: فعال (تا تاریخ: {i['VipMode']['date']})\n"
                # sh_msg += f"🔰بخش ویژه: فعال (مدت باقی مانده: {int(time() - i['VipMode']['pay'])})\n"
                # sh_msg += f"🔰بخش ویژه: فعال (تا تاریخ: {ctime(i['VipMode']['pay'])})\n"
...
def FinishVip():
    while True:
        for user in Users.find():
            if user['VipMode']['pay'] and user['VipMode']['End'] != 'Never':
                if user['VipMode']['End'] <= time():
                    Users.find_one_and_update({'_id':user['_id']}, {'$set':{'VipMode':{'pay':False, 'End':-9999}}})

Thread(target=FinishVip).start()
