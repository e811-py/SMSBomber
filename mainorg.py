...
admin_Mkeyboard = [['ارسال پیام به همه کاربران'],['مشاهده لیست کاربران', 'دانلود دیتابیس'],['بن کردن', 'آزاد کردن'],['ویژه کردن', 'عادی کردن'], ['برگشت']]
...
updater = Updater('1379182862:AAHU9yYnaRc-y2hn-gcQOu_s-5rcJs-sSF8', use_context=True)
...
def main_InsertToDB(Chat_id, name, viptest, targets, subset, license, banmood):
    ...
    for row in sql:
        if int(row[0]) == int(Chat_id):
            ...
            req = True
    if s == 0:
        c.execute(f''' INSERT INTO Users (ChatID, Name, VipTest, LTarget, Subset, License, BanMood, PCount) VALUES ("{Chat_id}", "{name}", "{viptest}", "{targets}", "{subset}", "{license}", "{banmood}", "0")''')
        conn.commit()
        bmm = False
        req = False
    return s
...
def start(update, context):
    ...
    if not bmm:
        ...
        if context.bot.get_chat_member(-1001144465509, update.message.chat_id).status == 'left':
            ...
            if update.message.chat_id == Admin_Chatid:
                update.message.reply_text('سلام\nبه ربات اس ام اس بمبر خوش آمدید.\n——————————————\nبرای مشاهده دستورات /help را ارسال کنید.', reply_markup=admin_Smarkup)
            else:
                update.message.reply_text('سلام\nبه ربات اس ام اس بمبر خوش آمدید.\n——————————————\nبرای مشاهده دستورات /help را ارسال کنید.', reply_markup=start_markup)
    elif bmm :
...
def Free_GetCount(update, context):
    ...
    try:
        if int(update.message.text) <= 0 or int(update.message.text) >= 100 :
            ...
            # file.write(jw)
            # file.close()
            update.message.reply_text(f'{count} اس ام اس به شماره {phone} ارسال شد.', reply_markup=Smenu_markup)
